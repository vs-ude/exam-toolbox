...
export class SearchComponent {
  ...
  private filterTags() {
    ...
    this.api.getAllTags().subscribe(
      tags => {
        ...
        this.filteredTagsWithTasks = filteredTags.map(tag => ({
          tag,
          tasks: [],
        }));
  ...
  private filterTasks() {
    ...
    this.api.getTasksWithQuestionTextFromPool(searchString).subscribe(
      ...
      err => {
        (console.error('Error fetching Tasks by question text'), err);
      },
    );
  }
  ...
  private filterExams() {
    ...
    this.api.getExamsWithSearchText(searchString).subscribe(
      ...
      err => {
        (console.error('Error fetching Exams by search text'), err);
      },
    );
  }
