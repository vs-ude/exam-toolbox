...
// @ts-ignore
import { MongoClient, Collection, Database } from "https://deno.land/x/mongo/mod.ts";
...
import { updateMetaStudent, generateExam, updateMetaTemplate, generateTasksLatex } from "./generation.ts";
...
export function configureRouter({ db, jobs, taskQueue, workers, basePath, JOBS_DIR, processQueue, genRandomNumber, getDownloadableJobs }: AppState): Router {
  ...
  router
    ...
    .post("/api/upload", async (ctx) => {
      ...
      const fileTrackerEntry: any = { name: file.originalName, refs: [], timeToLive: 7 }
      try {
        ...
        for (const [jobId, existingJob] of jobs.entries()) {
          if (existingJob.examId === examId) {
            if (['processing', 'queued', 'finalizing'].includes(existingJob.status)) {
              console.log(`Blocked new job request for exam ${examId} because job ${existingJob.jobId} is already active.`);
              ctx.response.status = 409;
              ctx.response.body = { message: `A generation job for this exam is already in progress. Please wait for it to complete.` };
              return;
            }
            console.log(`Found old, finished job ${existingJob.jobId} (status: ${existingJob.status}) for exam ${examId}. Replacing it.`);
            await Deno.remove(existingJob.jobDir, { recursive: true }).catch(err => {
              console.error(`Error cleaning up old job directory ${existingJob.jobId}:`, err);
            });
            jobs.delete(jobId);
            break;
          }
