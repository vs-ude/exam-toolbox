  ...
  const config = {
    ...
    auth: {
      ldap: {
        ...
        bindPassword:
          env('AUTH_LDAP_BIND_PASSWORD') ?? base.auth.ldap.bindPassword,
        searchBase: env('AUTH_LDAP_SEARCH_BASE') ?? base.auth.ldap.searchBase,
        searchFilter:
          env('AUTH_LDAP_SEARCH_FILTER') ?? base.auth.ldap.searchFilter,
        groupDnBase:
          env('AUTH_LDAP_GROUP_DN_BASE') ?? base.auth.ldap.groupDnBase,
        groups: {
          required:
            env('AUTH_LDAP_GROUPS_REQUIRED') ?? base.auth.ldap.groups.required,
    ...
    paths: {
      templateBase: env('PATHS_TEMPLATE_BASE_PATH') ?? base.paths.templateBase,
...
export function getConfig() {
  ...
  if (!appConfig) {
    const configPath =
      Deno.env.get('CONFIG') || flags.conf || '../../config.yaml';
    let raw = '';
    try {
